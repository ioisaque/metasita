...
class UniteCreatorMappickerView{
	...
	private function initSettings(){
		...
		$settings->addSap(__("Size and Location", ADDONLIBRARY_TEXTDOMAIN));
		
		$settings->addTextBox("width","100%",__("Map Width",ADDONLIBRARY_TEXTDOMAIN),array("unit"=>"px or %"));
		$settings->addTextBox("height","300",__("Map Height",ADDONLIBRARY_TEXTDOMAIN),array("unit"=>"px or %"));
		...
		$settings->addButton("button_my_location", __("Goto My Location", ADDONLIBRARY_TEXTDOMAIN), UniteSettingsUC::PARAM_NOTEXT, $params);
		
		//--------- add marker
		
		$settings->addSap(__("Marker", ADDONLIBRARY_TEXTDOMAIN));
		...
		$settings->addImage("marker_image","",__("Marker Image",ADDONLIBRARY_TEXTDOMAIN));
		$settings->addControl("marker_type", "marker_image", "show", "image");
		
		
		$settings->addSap(__("Style", ADDONLIBRARY_TEXTDOMAIN));
		...
		$settings->addSap(__("Language and Controls", ADDONLIBRARY_TEXTDOMAIN));
		
		//--- language
		$settings->addSelect("language", $this->arrLangs, __("Language", ADDONLIBRARY_TEXTDOMAIN), "");
		...
		?>	
			<div class="uc-mappicker-panels-wrapper">
				...
				<div class="uc-mappicker-overlay-text">
					<?php _e("For edit the map, please enter your google map API Key in", ADDONLIBRARY_TEXTDOMAIN)?> <?php echo $linkGeneralSettings?>.
					<br>
					<br>
				...
				<div class="unite-panels-wrapper unite-clearfix">
					...
					<div class="unite-right-panel">
						...
						<div id="uc_mappicker_mapwrapper" <?php echo $addHtml?> class="uc-mappicker-wrapper">
	...
	private function outputScripts(){
		...
		?>
			<script type="text/javascript" src="<?php echo $this->urlScript?>"></script>
			<script id="uc_mappicker_script" src="<?php echo $urlGoogleScript?>"></script>
			...
			<script>
				...
				jQuery(document).ready(function(){
					...
					g_objMapPicker.initMap("<?php echo $apiKey?>");
				});
	...
	public function putPickerInputHtml(){
		...
		$status = __("No Map Chosen", ADDONLIBRARY_TEXTDOMAIN);
		...
		?>
			<div class="unite-mappicker-chooser-wrapper">
				<img class="unite-mappicker-mapimage" src="<?php echo $urlImage?>" width="<?php echo $width?>" height="<?php echo $height?>">
	        	<div class="unite-mappicker-chooser-overlay"></div>
	        	<a href="javascript:void(0)" class="unite-mappicker-button unite-button-secondary unite-center-position" ><?php _e("Choose Map", ADDONLIBRARY_TEXTDOMAIN)?></a>
			</div>
	...
	private function putHtmlClientSide(){
		...
		<script type="text/javascript">
			function <?php echo $functionName?>(){
				...
				var strOptions = '<?php echo $jsonOptions?>';
				...
				<?php if($style):?>
				var strStyles = '<?php echo $style?>';
				mapOptions.styles = JSON.parse(strStyles);
				<?php endif?>

				<?php if($mapTypeID):?>
				mapOptions.mapTypeId = "<?php echo $mapTypeID?>";
				<?php endif?>
				...
				marker = new google.maps.Marker({
					...
					map:map
				<?php if($markerIcon):?>,icon:"<?php echo $markerIcon?>"<?php endif?>
				});
				<?php endif?>
			}
			...
			if(typeof g_ucGoogleMapLoading == "undefined" && (typeof google == "undefined" || typeof google.maps == "undefined")){
				...
				if(typeof g_ucGoogleMapLoaded != "undefined" && g_ucGoogleMapLoaded == true)
					...
					g_interval_<?php echo $mapID?> = setInterval(function(){
						if(typeof g_ucGoogleMapLoaded != "undefined" && g_ucGoogleMapLoaded == true){
							clearInterval(g_interval_<?php echo $mapID?>);
							<?php echo $functionName?>();
						}
					},200);
