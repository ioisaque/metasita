...
class UniteCreatorAssetsWork extends UniteCreatorAssets{
	...
	private function moveFilesFromData($data){
		...
		if(!empty($message)){
			...
			$response["html"] = $html;
			$response["message"] = __("files moved successfully", ADDONLIBRARY_TEXTDOMAIN);
		}
		return($response);
	}
	...
	private function checkAjaxActions_editor($action, $data){
		...
		switch($action){
			...
			case "assets_save_file":
				...
				HelperUC::ajaxResponseSuccess(__("File Saved", ADDONLIBRARY_TEXTDOMAIN));
			break;
