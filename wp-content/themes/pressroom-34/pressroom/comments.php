...
if(have_comments()):
	...
	//$comments_count = (get_option('comment_moderation') ? $comments_count->approved : $comments_count->total_comments);
	...
	<ul id="comments_list">
		...
		wp_list_comments(array(
			...
			'post_id' => get_the_ID(),
			'status' => (get_option('comment_moderation') ? 'approve' : 'all')
		))*/);
		?>
	</ul>
	<?php
...
?>
	<li <?php comment_class('comment clearfix'); ?> id="comment-<?php comment_ID() ?>">
		...
		<div class="comment_details">
			...
			<?php 
			if(get_option('comment_moderation') && $comment->comment_approved == '0')
				echo apply_filters("comment_text", __('Comment is awaiting moderation.', 'pressroom'));
			else
				comment_text(); 
...
function pr_comments_pagination($range, $pages)
{
	...
	if($paged > 1 && $showitems < $pages) echo "<li class='left'><a href='#page-" . esc_attr(($paged-1)) . "'>&nbsp;</a></li>";

	for ($i=1; $i <= $pages; $i++)
	{
		if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems ))
		{
			echo "<li" . ($paged == $i ? " class='selected'" : "") . ">" . ($paged == $i ? "<span>".$i."</span>":"<a href='#page-" . esc_attr($i) . "'>".$i."</a>") . "</li>";
		}
	}

	if ($paged < $pages && $showitems < $pages) echo "<li class='right'><a href='#page-" . esc_attr(($paged+1)) . "'>&nbsp;</a></li>";  
