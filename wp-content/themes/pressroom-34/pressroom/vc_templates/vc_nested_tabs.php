...
// Extract tab titles
preg_match_all( '/vc_nested_tab title="([^\"]+)"(\stab_id\=\"([^\"]+)\"){0,1}/i', $content, $matches, PREG_OFFSET_CAPTURE );
$tab_titles = array();
...
foreach ( $tab_titles as $tab ) {
	preg_match('/title="([^\"]+)"(\stab_id\=\"([^\"]+)\"){0,1}/i', $tab[0], $tab_matches, PREG_OFFSET_CAPTURE );
	if(isset($tab_matches[1][0])) {
		$href = (isset($tab_matches[3][0]) ? $tab_matches[3][0] : sanitize_title( $tab_matches[1][0] ) );
		if(substr($href, 0, 4)!="http" && substr($href, 0, 5)!="https")
			$href = "#" . $href;
		$tabs_nav .= '<li><a href="'. esc_attr($href) .'">' . $tab_matches[1][0] . '</a></li>';

	}
}
$tabs_nav .= '</ul>'."\n";

$output .= '<div class="clearfix tabs'.(isset($width) ? $width : '').esc_attr($el_class).($top_margin!="none" ? ' ' . esc_attr($top_margin) : '').'" data-interval="'.esc_attr($interval).'">';
$output .= "\n\t\t\t".$tabs_nav;
