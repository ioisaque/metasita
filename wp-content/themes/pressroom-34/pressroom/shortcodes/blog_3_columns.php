	...
	if(have_posts())
	{
		...
		while (have_posts()) : the_post();
			...
			if($i==0 || ((($i%$rows==0 || $rows==1) && $featured_post=="-") || ($featured_post!="-" && $i==1)))
			{
				...
				$output .= '<div class="vc_col-sm-4 wpb_column vc_column_container"><ul class="blog three_columns clearfix' . ($featured_post!="-" && $i==1 ? " small" : "") . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . ($el_class!="" ? ' ' . esc_attr($el_class) : '') . '">';
			}
