...
function pr_theme_single_post($atts, $content)
{
	...
	$output .= '">';
	if($show_post_title) 
		$output .= '<h1 class="post_title"><a href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a></h1>';
	
	if(1 == 0 && (int)$show_post_categories || (int)$show_post_date || (int)$show_post_author || (int)$show_post_views || (int)$show_post_comments)
	{
		...
		if((int)$show_post_author)
			$output .= '<li class="detail author"><a class="author" href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr(get_the_author()) . '">' . get_the_author() . '</a>';
		...
		{	
			...
			$output .= '<li class="detail comments scroll_to_comments"><a href="' . esc_url(get_comments_link()) . '" title="' . esc_attr($comments_count . ' ' . ($comments_count==1 ? __('comment', 'pressroom') : __('comments', 'pressroom'))) . '">' . $comments_count . ' ' . ($comments_count==1 ? __('comment', 'pressroom') : __('comments', 'pressroom')) . '</a></li>';
		}
		$output .= '</ul>';
	}
	$is_review = get_post_meta($post->ID, $themename. "_is_review", true);
	if(1 == 0 && $post_format=="quote")
	{
	...
	if(1 == 0 && $post_format=="video" && get_post_meta($post->ID, $themename. "_video_url", true)!="")
	{
	...
	else if(1 == 0 && $post_format=="gallery")
	{
			...
			<div class="header_container padding_top_bottom_15">
				<div class="header clearfix">';
					if($theme_options["logo_text"]!="")
						$output .= '<h1><a title="' . esc_attr($theme_options["logo_text"]) . '" href="' . esc_url(get_home_url()) . '">' . $theme_options["logo_text"] . '</a></h1>';
		...
		if(count($icon_type))
		{
			...
			for($i=0; $i<count($icon_type); $i++)
			{
				...
				$icons_html .= '<li><a' . ($icon_target[$i]=="new_window" ? ' target="_blank"' : '') . ' title="" href="' . esc_url(str_replace("{MEDIA_URL}", $large_image_url, str_replace("{TITLE}", urlencode(get_the_title()), str_replace("{URL}", esc_url(get_permalink()), $icon_url[$i])))) . '" class="social_icon ' . esc_attr($icon_type[$i]) . '">&nbsp;</a></li>';
			}									
			$icons_html .= '</ul></div>';
		}
		/*if(is_rtl())
	...
	{
		$output .= '<div itemprop="review" itemscope itemtype="http://schema.org/Review">
					...
					<div class="value_container">
						<div class="value_bar_container" style="width:' . ($review_value[$i]/$max*100) . '%;">
							<div class="value_bar animated_element animation-width duration-2000">
								<span class="number ' . ($is_review=="percentage" ? 'animated_element' : 'tens') . '" data-value="' . esc_attr($review_value[$i]) . '"></span>
							</div>
						</div>
					</div>
				</li>';
		...
		{
			$output .= '<div class="review_summary margin_top_10 clearfix" itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">
				<div class="number' . ($is_review=="points" ? ' tens' : '') . '" data-value="' . esc_attr($review_average) . '">' . ($is_review=="percentage" ? $review_average . '%' : '') . '</div>
				<div class="text">
					<h4 class="margin_top_0">' . $review_title . '</h4>
					<p itemprop="description">' . $review_description . '</p>
				</div>
				<meta itemprop="worstRating" content="0">
				<meta itemprop="bestRating" content="' . esc_attr($max) . '">
				<meta itemprop="ratingValue" content="' . esc_attr($review_average) . '">
				<div class="value_bar_container" style="width:' . esc_attr($review_average/$max*100) . '%;"><div class="value_bar animated_element duration-2000 animation-width"></div></div>
			</div>';
		}
		$output .= '</div>';
	}
	...
	if(((int)$show_post_excerpt==1) || ((int)$show_post_excerpt==2 && has_excerpt()))
		$output .= '<h3 class="excerpt">' . apply_filters("the_excerpt", get_the_excerpt()) . '</h3>';	
	$output .= '<div class="text">' . (function_exists("wpb_js_remove_wpautop") ? wpb_js_remove_wpautop(apply_filters('the_content', get_the_content())) : apply_filters('the_content', get_the_content()));
	...
	if(1 == 0 && (int)$show_post_comments && (int)$comments_count)
		$output .= '<a class="read_more page_margin_top scroll_to_comments" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr($comments_count) . ' ' . ($comments_count==1 ? __('comment', 'pressroom') : __('comments', 'pressroom')) . '"><span class="arrow"></span><span>' . $comments_count . ' ' . ($comments_count==1 ? __('comment', 'pressroom') : __('comments', 'pressroom')) . '</span></a>';
	$output .= '</div>';
	if($post_format!="image")
		$output .= '</div>';
	if(1 == 0 && (int)$show_post_author_box)
	{
		$author = get_user_by("id", get_the_author_meta('ID'));
		$output .= '<div class="author_box animated_element">
			<div class="single-author">
				<a class="thumb" href="' . esc_url(get_author_posts_url($author->ID)) . '" title="' . esc_attr($author->display_name) . '">' . get_avatar($author->ID, 100) . '</a>
				<div class="details">
					<h5><a href="' . esc_url(get_author_posts_url($author->ID)) . '" title="' . esc_attr($author->display_name) . '">' . $author->display_name . '</a></h5>
					<h6>' . (isset($author->roles[0]) ? strtoupper($author->roles[0]) : '') . '</h6>
					<a href="' . esc_url(get_author_posts_url($author->ID)) . '" class="more highlight margin_top_15">' . __("PROFILE", 'pressroom') . '</a>
				</div>
			</div>
		</div>';
	}
	if($post_format=="image")
		$output .= '</div>';
	$output .= '</div></div>';
	if(1 == 0 && $show_share_box && (int)$icons_count)
	{
		...
		for($i=0; $i<$icons_count; $i++)
		{
			if(isset($atts["icon_target" . $i]))
			{
				...
				$output .= '<li><a' . ($atts["icon_target" . $i]=="_blank" ? ' target="_blank"' : '') . ' title="" href="' . esc_url(str_replace("{MEDIA_URL}", $large_image_url, str_replace("{TITLE}", urlencode(get_the_title()), str_replace("{URL}", esc_url(get_permalink()), $atts["icon_url" . $i])))) . '" class="social_icon ' . esc_attr($atts["icon_type" . $i]) . '">&nbsp;</a></li>';
			}
		}
		$output .= '</ul></div></div>';
	}
	if(1 == 0 && (int)$show_post_tags_footer || (int)$show_post_categories_footer)
	{
		$output .= '<div class="vc_row wpb_row vc_row-fluid page_margin_top">';
		if((int)$show_post_tags_footer)
		{
			...
			if($tags)
			{
				...
				{
					$output .= '<li><a href="' . esc_url(get_tag_link($tag->term_id )) . '" ';
					if(empty($tag->description))
						$output .= 'title="' . sprintf(__('View all posts filed under %s', 'pressroom'), esc_attr($tag->name)) . '"';
					else
	...
	$output .= '<div class="author_box page_margin_top clearfix">
		<div class="single-author">
			<a class="thumb" href="' . esc_url(get_author_posts_url($author->ID)) . '" title="' . esc_attr($author->display_name) . '" style="float: left;">' . get_avatar($author->ID, 100) . '</a>
