		...
		while (have_posts()) : the_post();
			...
			if($i==0 || (($i==ceil($post_count/2) && $featured_post=="-") || ($featured_post!="-" && $i==1)))
			{
				if(($i==ceil($post_count/2) && $featured_post=="-") || ($featured_post!="-" && $i==1))
					$output .= '</ul></div>';
				$output .= '<div class="vc_col-sm-6 wpb_column vc_column_container"><ul class="blog clearfix' . ($featured_post!="-" && $i==1 ? " small" : "") . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . ($el_class!="" ? ' ' . esc_attr($el_class) : '') . '">';
			}
			...
			if((int)$show_post_title)
			{
				...
				$output .= ($featured_post!="-" && $i>0 ? '<h5>' : '<h3 class="clearfix' . ((int)$show_post_comments_box ? ' with_number' : '') . '">') . '<a href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a>' . ((int)$show_post_comments_box && ($featured_post=="-" || $i==0)? '<a href="' . esc_url(get_comments_link()) . '" title="' . esc_attr($comments_count) . ' ' . ($comments_count==1 ? __('comment', 'pressroom') : __('comments', 'pressroom')) . '" class="comments_number">' . $comments_count . '<span class="arrow_comments"></span></a>' : '') . ($featured_post!="-" && $i>0 ? '</h5>' : '</h3>');
			}
