<?php
//blog
function pr_theme_blog_medium($atts, $content="", $component="blog_medium")
{
	global $themename;
	if(isset($_POST["atts"]))
	...
	if(have_posts())
	{
		...
		while (have_posts()) : the_post();
			$post = new Pr_Post("medium", $featured_post, get_post_meta(get_the_ID(), $themename. "_is_review", true), get_post_format(get_the_ID()), $featured_image_size, (int)$show_post_icon, (int)$show_post_date, (int)$show_post_categories, (int)$show_post_excerpt, (int)$show_post_author, $post_details_layout, $post_list_style, $category, $i, $themename);
			if($i==0 || ($featured_post!="-" && $i==1))
			{
				if($featured_post!="-" && $i==1)
					$output .= '</ul>';
				$output .= '<ul class="blog clearfix' . ($featured_post!="-" && $i==1 ? (substr($post_list_style,0,6)=="simple" ? " list" : " small") : " small_margin") . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . ($el_class!="" ? ' ' . esc_attr($el_class) : '') . '">';
			}
			$output .= $post->getLiCssClass();
			if($featured_post!="-" && $i>0 && substr($post_list_style,0,6)=="simple")
			{
				$output .= '<a href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a>';
			}
			else
			{
				$output .= $post->getThumbnail("blog-post-thumb-medium");
				...
				if((int)$show_post_title)
				{
					...
					$output .= '<h5 class="clearfix' . ((int)$show_post_comments_box && ($featured_post=="-" || $i==0) ? ' with_number' : '') . '"><a href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a>' . ((int)$show_post_comments_box && ($featured_post=="-" || $i==0) ? '<a href="' . esc_url(get_comments_link()) . '" title="' . esc_attr($comments_count) . ' ' . ($comments_count==1 ? __('comment', 'pressroom') : __('comments', 'pressroom')) . '" class="comments_number">' . $comments_count . '<span class="arrow_comments"></span></a>' : '') . '</h5>';
				}
...
//ajax pagination
add_action("wp_ajax_theme_blog_medium_pagination", "pr_theme_blog_medium");
add_action("wp_ajax_nopriv_theme_blog_medium_pagination", "pr_theme_blog_medium");

//visual composer
function pr_theme_blog_medium_vc_init()
{
	//get posts list
	...
	vc_map( array(
		...
		"params" => array(
			...
			array(
				"type" => "dropdown",
				"class" => "",
				"heading" => __("Other (not featured) posts list style", 'pressroom'),
				"param_name" => "post_list_style",
				"value" => array(__("Default", 'pressroom') => 'default', __("Simple list bullet style 1", 'pressroom') => 'simple_bullet_1', __("Simple list bullet style 2", 'pressroom') => 'simple_bullet_2', __("Simple list bullet style 3", 'pressroom') => 'simple_bullet_3', __("Simple list bullet style 4", 'pressroom') => 'simple_bullet_4'),
...
add_action("init", "pr_theme_blog_medium_vc_init");
?>
