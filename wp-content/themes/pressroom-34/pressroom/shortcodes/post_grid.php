...
function pr_theme_post_grid_shortcode($atts)
{
	...
	{
		$output .= '<div class="post ' . esc_attr($type) . '">
						<a href="' . esc_url(get_permalink($post->ID)) . '" title="' . esc_attr($post->post_title) . '">';
						...
						$output .= get_the_post_thumbnail($post->ID, "post-grid-thumb-" . ($type=="small" ? "big" : $type), array("alt" => get_the_title(), "title" => "")) .
						'</a>
						<div class="slider_content_box">';
							...
							if((int)$show_post_categories || (int)$show_post_date)
								$output .= '</ul>';
							$output .= '<h' . ($type=="small" ? '5' : '2') . '><a href="' . esc_url(get_permalink($post->ID)) . '" title="' . esc_attr($post->post_title) . '">' . $post->post_title . '</a></h' . ($type=="small" ? '5' : '2') . '>
						</div>
					</div>';
	}
	return $output;
