<?php
//blog
function pr_theme_blog_big($atts, $content="", $component="blog_big")
{
	global $themename;
	if(isset($_POST["atts"]))
	...
	if(have_posts())
	{
		...
		while (have_posts()) : the_post();
			$post = new Pr_Post("big", $featured_post, get_post_meta(get_the_ID(), $themename. "_is_review", true), get_post_format(get_the_ID()), $featured_image_size, (int)$show_post_icon, (int)$show_post_date, (int)$show_post_categories, (int)$show_post_excerpt, (int)$show_post_author, $post_details_layout, "", $category, $i, $themename);
			if($i==0 || ($featured_post!="-" && $i==1))
			{
				if($featured_post!="-" && $i==1)
					$output .= '</ul>';
				$output .= '<ul class="blog clearfix' . ($featured_post!="-" && $i==1 ? " small" : " big") . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . ($el_class!="" ? ' ' . esc_attr($el_class) : '') . '">';
			}
			...
			if((int)$show_post_title)
			{
				...
				$output .= ($featured_post!="-" && $i>0 ? '<h5>' : '<h2 class="clearfix' . ((int)$show_post_comments_box ? ' with_number' : '') . '">') . '<a href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a>' . ((int)$show_post_comments_box && ($featured_post=="-" || $i==0)? '<a href="' . esc_url(get_comments_link()) . '" title="' . esc_attr($comments_count) . ' ' . ($comments_count==1 ? __('comment', 'pressroom') : __('comments', 'pressroom')) . '" class="comments_number">' . $comments_count . '<span class="arrow_comments"></span></a>' : '') . ($featured_post!="-" && $i>0 ? '</h5>' : '</h2>');
			}
...
//ajax pagination
add_action("wp_ajax_theme_blog_big_pagination", "pr_theme_blog_big");
add_action("wp_ajax_nopriv_theme_blog_big_pagination", "pr_theme_blog_big");

//visual composer
function pr_theme_blog_big_vc_init()
{
	//get posts list
...
add_action("init", "pr_theme_blog_big_vc_init");
?>
